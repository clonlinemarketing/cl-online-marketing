  ...
  return (
    <>
      ...
      <section className="py-20 md:py-28 px-6 overflow-hidden" style={{ background: '#FFFFFF' }}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={staggerContainer} {...safeAnimAnimate(staggerContainer, shouldReduce)}>
            <motion.div variants={fadeUp}><Eyebrow>Digital Pay Ads Malaysia</Eyebrow></motion.div>
            ...
            <motion.p className="text-lg mb-8" style={{ color: '#4A5568' }} variants={fadeUp}>Google Ads and social media advertising that puts your business in front of high-intent Malaysian buyers on the channels they use most — measurable ROI from day one.</motion.p>
      ...
      <section className="py-20 px-6" style={{ background: '#FFFFFF' }}>
        <div className="max-w-7xl mx-auto">
          <motion.div className="text-center mb-12" variants={fadeUp} initial="hidden" whileInView="visible" viewport={viewportOnce}><Eyebrow>Why CL</Eyebrow><h2 className="mt-3">Why Choose CL for Your Paid Ads.</h2></motion.div>
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce}>
            {[{ icon: Target, title: 'Built to Convert, Not Just Click', body: 'We optimise for the metrics that matter — leads, calls, and sales — not just impressions and clicks.' }, { icon: BarChart3, title: 'Transparent Reporting', body: 'Clear monthly reports showing exactly where your budget went and what it delivered.' }, { icon: Globe, title: 'Malaysia-Market Experience', body: 'We know what works for Malaysian audiences — the platforms, messaging, and targeting that converts.' }].map((t) => (<motion.div key={t.title} variants={fadeUp}><TrustCard {...t} /></motion.div>))}
          </motion.div>
        </div>
      </section>
      {/* STATS */}
      <section className="py-16 px-6" style={{ background: '#F5F7FA' }}>
        <div className="max-w-7xl mx-auto">
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce}>
            {[{ stat: '2', label: 'Paid Ad Platforms Managed' }, { stat: '50+', label: 'Malaysian Clients Served' }, { stat: '5+', label: 'Years Paid Ads Experience' }].map((s) => (<motion.div key={s.label} variants={scaleIn}><StatCard {...s} /></motion.div>))}
          </motion.div>
        </div>
      </section>
