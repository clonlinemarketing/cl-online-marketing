  ...
  return (
    <>
      ...
      <section className="py-20 md:py-28 px-6 overflow-hidden" style={{ background: '#FFFFFF' }}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={staggerContainer} {...safeAnimAnimate(staggerContainer, shouldReduce)}>
            <motion.div variants={fadeUp}><Eyebrow>Google Search Engine & AI Search Malaysia</Eyebrow></motion.div>
      ...
      <section className="py-20 px-6" style={{ background: '#F5F7FA' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce}>
            {[
              { icon: Search, title: 'Google SEO', body: 'First-page Google rankings for the keywords your Malaysian customers search. Long-term organic growth that compounds month after month.', to: '/services/search/google-seo' },
      ...
      <section className="py-16 px-6" style={{ background: '#F5F7FA' }}>
        <div className="max-w-7xl mx-auto">
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewportOnce}>
            {[{ stat: '2', label: 'Search Disciplines (SEO + AI Search)' }, { stat: '5+', label: 'Years SEO Experience' }, { stat: '50+', label: 'Malaysian Clients Served' }].map((s) => (<motion.div key={s.label} variants={scaleIn}><StatCard {...s} /></motion.div>))}
          </motion.div>
        </div>
      </section>
      <CTABanner headline="Ready to Dominate Google and AI Search in Malaysia?" cta1="Book a Call" />
    </>
  );
}
